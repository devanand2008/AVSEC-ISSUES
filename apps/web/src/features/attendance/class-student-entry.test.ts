...
describe("attendance class student entry", () => {
  it("requires the official email and register number expected by the API", () => {
    expect(
      validateAttendanceStudentDraft({ ...validDraft(), email: "" }),
    ).toMatch(/official college email/i);
    expect(
      validateAttendanceStudentDraft({ ...validDraft(), registerNumber: "" }),
    ).toMatch(/register number/i);
  });
