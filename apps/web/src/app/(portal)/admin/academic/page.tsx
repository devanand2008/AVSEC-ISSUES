...
interface Programme { id: string; code: string; name: string; degreeTypeId: string | null; degreeTypeMaster: { id: string; code: string; name: string } | null; durationYears: number; totalSemesters: number; isActive: boolean; department: { id: string; name: string }; _count: { semesters: number; studentProfiles: number } }
...
export default function AcademicAdminPage() {
  ...
  return <>
    <PageHeader
      ...
      actions={
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {tab === "programmes" && (
            <Link className="btn btn-secondary" href="/admin/academic/programmes">
              Programme Master
            </Link>
          )}
    ...
    {creating && tab !== "assignments" && !["programmes", "years", "sections"].includes(tab) && <CreateForm tab={tab} onCreated={() => setCreating(false)} />}
    {creating && tab === "programmes" && <div className="card" style={{ padding: 20, marginBottom: 18 }}><p>Create Programmes in the canonical Programme Master so every record has a database Degree Type.</p><Link className="btn btn-primary" href="/admin/academic/programmes">Open Programme Master</Link></div>}
    {creating && tab === "years" && <div className="card" style={{ padding: 20, marginBottom: 18 }}><p>Create Academic Years in the canonical master so date ranges, current-year uniqueness, and archive state are validated consistently.</p><Link className="btn btn-primary" href="/admin/academic/academic-years">Open Academic Year Master</Link></div>}
    {creating && tab === "sections" && <div className="card" style={{ padding: 20, marginBottom: 18 }}><p>Create Sections in Departments & Sections so Study Year and Semester compatibility are checked before saving.</p><Link className="btn btn-primary" href="/admin/academic/departments-sections">Open Departments & Sections</Link></div>}
...
function ProgrammesTable() {
  ...
  if (query.isLoading) return <LoadingState />;
  if (query.isError) return <ErrorState />;
  return <div className="card"><table className="data-table"><thead><tr><th>Code</th><th>Programme</th><th>Degree</th><th>Department</th><th>Duration</th><th>Semesters</th><th>Students</th><th>Status</th></tr></thead><tbody>{query.data?.map((item) => <tr key={item.id}><td><code>{item.code}</code></td><td>{item.name}</td><td>{item.degreeTypeMaster ? `${item.degreeTypeMaster.code} - ${item.degreeTypeMaster.name}` : "Not mapped"}</td><td>{item.department.name}</td><td>{item.durationYears} years</td><td>{item._count.semesters} / {item.totalSemesters}</td><td>{item._count.studentProfiles}</td><td><ToggleButton entityType="programmes" id={item.id} isActive={item.isActive} /></td></tr>)}</tbody></table>{!query.data?.length && <div className="empty">No programmes yet.</div>}</div>;
}
...
function CreateForm({ tab, onCreated }: { tab: StructureTab; onCreated: () => void }) {
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      if (tab === "programmes") throw new Error("Use the canonical Programme Master.");
      if (tab === "years") throw new Error("Use the canonical Academic Year Master.");
      ...
      if (tab === "sections") throw new Error("Use Departments & Sections for validated Section creation.");
