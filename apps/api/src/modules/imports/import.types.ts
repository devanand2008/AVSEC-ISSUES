...
export interface ImportRow {
  ...
  /** Internal aggregate row identity; never sourced from or exposed to operators. */
  import_row_number?: string;
  ...
  assigned_semester: string;
  assigned_section: string;
  assigned_block: string;
  assigned_floor: string;
  assigned_room: string;
  specialization: string;
  assigned_issue_category: string;
  shift: string;
  ...
  team_code: string;
  is_primary: string;
  max_open_issues: string;
  category_code: string;
  issue_type_code: string;
  asset_code: string;
  priority: string;
  primary_user_identity: string;
  backup_user_identity: string;
  escalation_user_identity: string;
  rule_priority: string;
  workload_balancing: string;
  subject_code: string;
  session_date: string;
  period_number: string;
  faculty_identity: string;
  status: string;
  note: string;
  marked_by: string;
}
...
export function importRowNumber(
  ...
  const aggregateRowNumber = Number(row?.import_row_number);
  if (Number.isInteger(aggregateRowNumber) && aggregateRowNumber >= 2) {
    return aggregateRowNumber;
  }
...
export const IMPORT_TEMPLATES: Record<ImportEntityType, ImportTemplate> = {
  ...
  STUDENTS: {
    ...
    example: {
      full_name: "Sample Student One",
      official_email: "sample1@example.edu",
      college_id: "AVS26CSE001",
      ...
      semester: "5",
      section: "A",
      temporary_password: "AvsTemp@2026!",
      mobile: "9876543210",
    },
  },
  STAFF: {
    ...
    example: {
      full_name: "Sample Faculty One",
      email: "faculty1@example.edu",
      temporary_password: "005678",
      role_codes: "FACULTY",
      department_code: "CSE",
      account_status: "ACTIVE",
    },
  ...
  DEPARTMENTS: {
    ...
    required: ["code", "name"],
    optional: ["campus_code", "sort_order"],
    example: {
      code: "CSE",
  ...
  ATTENDANCE: {
    ...
    example: {
      ...
      period_number: "3",
      faculty_identity: "FAC101",
      student_id: "2025CSE001",
      legacy_id: "LEGACY-0001",
      status: "P",
      note: "Migrated from legacy register",
      marked_by: "Legacy Clerk",
    },
  },
  BLOCKS: {
    entityType: "BLOCKS",
    permission: "locations.import",
    required: ["campus_code", "code", "name"],
    optional: ["sort_order"],
    example: {
      campus_code: "MAIN",
      code: "A",
  ...
  FLOORS: {
    entityType: "FLOORS",
    permission: "locations.import",
    required: ["campus_code", "block_code", "code", "name", "level"],
    optional: ["sort_order"],
    example: {
      campus_code: "MAIN",
      block_code: "A",
  ...
  ASSETS: {
    ...
    required: ["room_code", "category_name", "code", "name"],
    optional: ["serial_number", "installed_on"],
    example: {
      room_code: "A-101",
  ...
  RESPONSIBLE_PERSONS: {
    ...
    required: ["team_code", "college_identity_id"],
    optional: ["is_primary", "max_open_issues"],
    example: {
      team_code: "ELECTRICAL",
      college_identity_id: "EMP-201",
      is_primary: "true",
      max_open_issues: "12",
    },
  },
