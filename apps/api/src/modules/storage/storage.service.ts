...
import type {
  CompleteIssueAttachmentDto,
  CompleteMessageAttachmentUploadDto,
  CompleteModelQuestionPaperDto,
  CompleteProfilePhotoDto,
  CompleteSubjectResourceDto,
  PresignIssueAttachmentDto,
  PresignLearningFileDto,
  PresignMessageAttachmentDto,
  PresignProfilePhotoDto,
...
export type ManagedImageFolder = "campuses" | "blocks" | "floors" | "rooms";
export interface ManagedImageInput {
  fileName: string;
  mimeType: "image/jpeg" | "image/png" | "image/webp";
  sizeBytes: number;
}
export interface CompleteManagedImageInput extends ManagedImageInput {
  storageKey: string;
}
export interface ManagedImageCleanupInput {
  collegeId: string;
...
export class StorageService {
  ...
  async presign(
    user: AuthPrincipal,
    issueId: string,
    input: PresignIssueAttachmentDto,
    publicEndpoint?: string,
  ) {
    await this.requireIssue(user, issueId, true);
    if (
      !["ISSUE_REPORT", "ISSUE_UPDATE", "ISSUE_RESOLUTION"].includes(
        input.purpose,
      )
    )
      throw new BadRequestException(
        "Attachment purpose is not valid for an issue.",
      );
    ...
    const uploadUrl = await getSignedUrl(
      ...
      new PutObjectCommand({
        ...
        Metadata: { uploader: user.id, purpose: input.purpose },
      }),
      { expiresIn },
    );
    return {
  ...
  async presignManagedImage(
    user: AuthPrincipal,
    folder: ManagedImageFolder,
    entityId: string,
    input: ManagedImageInput,
    publicEndpoint?: string,
  ) {
    this.validateFile(input);
    const extension = extname(input.fileName).toLowerCase();
    const storageKey = `${this.managedImagePrefix(user.collegeId, folder, entityId)}${randomUUID()}${extension}`;
    const expiresIn = this.config.get<number>(
      "S3_SIGNED_URL_EXPIRY_SECONDS",
      300,
    );
  ...
  async prepareManagedImage(
    user: AuthPrincipal,
    folder: ManagedImageFolder,
    entityId: string,
    input: CompleteManagedImageInput,
  ) {
    this.validateFile(input);
    this.requireManagedImageKey(user, folder, entityId, input.storageKey);
    if (
      extname(input.storageKey).toLowerCase() !==
      extname(input.fileName).toLowerCase()
    ) {
      throw new ForbiddenException(
        "Storage key does not match the uploaded file name.",
      );
    }
  ...
  async managedImageUrls(
    ...
    this.requireManagedImageKey(user, folder, entityId, storageKey);
    const expiresIn = this.config.get<number>(
      "S3_SIGNED_URL_EXPIRY_SECONDS",
      300,
    );
    const signingClient = this.signingClient(publicEndpoint);
    ...
    return { imageUrl, thumbnailUrl, expiresIn };
  }

  async deleteManagedImageObjects(
    user: AuthPrincipal,
    folder: ManagedImageFolder,
    entityId: string,
    storageKey: string,
    ...
    this.requireManagedImageKey(user, folder, entityId, storageKey);
    return this.deleteMaintenanceObjects([
      storageKey,
      `${storageKey}.thumbnail.webp`,
    ]);
  }

  async deleteManagedImageObjectsIfUnreferenced(
    input: ManagedImageCleanupInput,
  ): Promise<{
    deleted: number;
    failed: number;
    skippedReferenced: boolean;
  }> {
    this.requireManagedImageKeyForCollege(
      input.collegeId,
      input.folder,
      input.entityId,
      input.storageKey,
    ...
    return this.prisma.$transaction(
      async (tx) => {
        const kind =
          input.folder === "campuses"
            ? "campus"
            : input.folder.slice(0, -1);
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`location-image:${input.collegeId}:${kind}:${input.entityId}`}))`;
        const currentStorageKey = await this.managedImageReference(
          tx,
          input.collegeId,
          input.folder,
          input.entityId,
        );
        if (currentStorageKey === input.storageKey) {
          return { deleted: 0, failed: 0, skippedReferenced: true };
        }
        return {
          ...(await this.deleteMaintenanceObjects([
            input.storageKey,
            `${input.storageKey}.thumbnail.webp`,
          ])),
  ...
  async completeProfilePhoto(
    ...
    if (!input.storageKey.startsWith(prefix)) {
      throw new ForbiddenException(
        "Storage key is outside the authorized profile path.",
      );
    }
    ...
    if (
      ...
      previous.profilePhotoKey !== input.storageKey
    ) {
      await this.deleteMaintenanceObjects([
        previous.profilePhotoKey,
  ...
  async deleteMaintenanceObjects(
    keys: string[],
  ): Promise<{ deleted: number; failed: number }> {
    let deleted = 0;
    let failed = 0;
    for (const key of [
      ...new Set(keys.filter((value) => value.startsWith("colleges/"))),
    ]) {
      try {
        await this.client.send(
  ...
  async complete(
    user: AuthPrincipal,
    issueId: string,
    input: CompleteIssueAttachmentDto,
    requestId: string,
  ) {
    await this.requireIssue(user, issueId, true);
    if (
    ...
    if (!input.storageKey.startsWith(prefix))
      throw new ForbiddenException(
        "Storage key is outside the authorized issue path.",
      );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(
        {
          actorId: user.id,
          action: "issue_attachment.created",
  ...
  async download(
    ...
    const attachment = await this.prisma.issueAttachment.findFirst({
      where: { id: attachmentId, issueId, deletedAt: null },
    });
    ...
    const url = await getSignedUrl(
      ...
      new GetObjectCommand({
        ...
        ResponseContentDisposition: `attachment; filename="${this.safeName(attachment.originalName)}"`,
      }),
      { expiresIn },
    );
  ...
  async presignPendingMessage(
    user: AuthPrincipal,
    input: PresignMessageAttachmentDto,
    publicEndpoint?: string,
  ) {
    await this.requireConversationWrite(user, input.conversationId);
    if (input.purpose !== "MESSAGE")
      throw new BadRequestException(
        "Message attachments must use the MESSAGE purpose.",
      );
    this.validateFile(input);
    ...
    const maxAttachments = this.config.get<number>(
      "MAX_ATTACHMENTS_PER_MESSAGE",
      10,
    );
    if (activeUploads >= maxAttachments)
      throw new BadRequestException(
        `A message can contain at most ${maxAttachments} attachments.`,
      );
    ...
    const expiresAt = new Date(
      Date.now() + Math.max(expiresIn + 300, 3600) * 1000,
    );
    const uploadUrl = await getSignedUrl(
      this.signingClient(publicEndpoint),
      new PutObjectCommand({
  ...
  async completePendingMessage(
    user: AuthPrincipal,
    uploadId: string,
    input: CompleteMessageAttachmentUploadDto,
    requestId: string,
  ) {
    await this.requireConversationWrite(user, input.conversationId);
    if (input.purpose !== "MESSAGE")
    ...
    if (!upload)
      throw new NotFoundException("Pending attachment upload not found.");
    if (upload.expiresAt <= new Date())
      throw new BadRequestException(
        "The attachment upload expired. Select the file and try again.",
      );
    if (
      upload.storageKey !== input.storageKey ||
      upload.originalName !== this.safeName(input.fileName) ||
      upload.mimeType !== input.mimeType ||
      upload.sizeBytes !== BigInt(input.sizeBytes)
    ) {
      throw new BadRequestException(
        "Attachment completion metadata does not match the reserved upload.",
      );
    }
    if (upload.status === "READY") return this.pendingAttachmentView(upload);
    if (!["UPLOADING", "FAILED"].includes(upload.status))
      throw new BadRequestException(
        "This attachment upload cannot be completed.",
      );
    try {
      const verified = await this.verifyObject(input);
      ...
      const ready = await this.prisma.$transaction(async (tx) => {
        ...
        await this.audit.record(
          {
            actorId: user.id,
            action: "message_attachment_upload.ready",
            entityType: "MessageAttachmentUpload",
            entityId: upload.id,
            afterValue: {
              conversationId: input.conversationId,
              mimeType: input.mimeType,
  ...
  async presignMessage(
    ...
    const uploadUrl = await getSignedUrl(
      ...
      new PutObjectCommand({
        ...
        ContentType: input.mimeType,
        ContentLength: input.sizeBytes,
        Metadata: { uploader: user.id, purpose: "MESSAGE" },
      }),
      { expiresIn },
    );
    return {
  ...
  async completeMessage(
    ...
    if (!input.storageKey.startsWith(prefix))
      throw new ForbiddenException(
        "Storage key is outside the authorized message path.",
      );
    const existingAttachments = await this.prisma.messageAttachment.count({
    ...
    if (existingAttachments >= maxAttachments)
      throw new BadRequestException(
        `A message can contain at most ${maxAttachments} attachments.`,
      );
    const verified = await this.verifyObject(input);
    ...
    const messageType = input.mimeType.startsWith("image/")
      ? "IMAGE"
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(
        {
          actorId: user.id,
          action: "message_attachment.created",
  ...
  async downloadMessage(
    ...
    const attachment = await this.prisma.messageAttachment.findFirst({
      where: { id: attachmentId, messageId, deletedAt: null },
    });
    if (!attachment)
  ...
  async completeSubjectResource(
    ...
    const targetSectionIds = input.targetSectionIds?.length
      ? input.targetSectionIds
  ...
  async remove(
    ...
    if (!attachment) throw new NotFoundException("Attachment not found.");
    const teamMember = issue.teamId
      ? Boolean(
          await this.prisma.responsibleTeamMember.findFirst({
            where: { teamId: issue.teamId, userId: user.id, isActive: true },
          }),
        )
      : false;
    const canWork = this.access.canWorkIssue(user, issue, teamMember);
    if (attachment.uploadedById !== user.id && !canWork)
      throw new ForbiddenException(
        "Only the uploader or authorized responsible staff may remove this attachment.",
      );
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(
        {
          actorId: user.id,
          action: "issue_attachment.removed",
          entityType: "IssueAttachment",
          entityId: attachment.id,
          beforeValue: {
            issueId,
          ...
          afterValue: { deletedAt: new Date() },
          reason: "Authorized attachment removal",
          requestId,
        },
        tx,
      );
  ...
  private signingClient(publicEndpoint?: string): S3Client {
    const endpoint =
      this.config.get<string>("S3_PUBLIC_ENDPOINT") ?? publicEndpoint;
    return endpoint
      ? new S3Client({ ...this.clientOptions, endpoint })
      : this.client;
  }

  private managedImagePrefix(
    collegeId: string,
    folder: ManagedImageFolder,
    entityId: string,
  ): string {
    return `colleges/${collegeId}/campus-images/${folder}/${entityId}/`;
  }

  private isManagedImageKey(storageKey: string): boolean {
    return /^colleges\/[0-9a-f-]+\/campus-images\/(campuses|blocks|floors|rooms)\/[0-9a-f-]+\/[0-9a-f-]+\.(jpe?g|png|webp)$/i.test(
      storageKey,
    );
  }

  private requireManagedImageKey(
    user: AuthPrincipal,
    folder: ManagedImageFolder,
    entityId: string,
    storageKey: string,
  ...
  private requireManagedImageKeyForCollege(
    collegeId: string,
    folder: ManagedImageFolder,
    entityId: string,
    storageKey: string,
    ...
    if (
      !storageKey.startsWith(
        this.managedImagePrefix(collegeId, folder, entityId),
      ) ||
      !this.isManagedImageKey(storageKey)
    ) {
      throw new ForbiddenException(
        "Storage key is outside the authorized location-image path.",
      );
    }
  }

  private async managedImageReference(
    ...
    if (folder === "campuses") {
      const record = await client.campus.findFirst({
        where: { id: entityId, collegeId },
        select: { imageStorageKey: true },
      });
      return record?.imageStorageKey ?? null;
    }
    if (folder === "blocks") {
      const record = await client.block.findFirst({
        where: { id: entityId, campus: { collegeId } },
        select: { imageStorageKey: true },
      });
      return record?.imageStorageKey ?? null;
    }
    if (folder === "floors") {
      const record = await client.floor.findFirst({
        where: { id: entityId, block: { campus: { collegeId } } },
        select: { imageStorageKey: true },
      });
      return record?.imageStorageKey ?? null;
    }
    const record = await client.room.findFirst({
      where: {
        id: entityId,
  ...
  private async requireSubjectUpload(
    ...
    if (!collegeAdmin) {
      if (user.roles.includes("HOD")) {
        ...
        const assignments = await this.prisma.facultySubjectAssignment.findMany(
          {
            where: {
              facultyId: user.id,
              subjectId,
  ...
  private async requireIssue(
    ...
    const issue = await this.prisma.issue.findFirst({
      ...
      select: {
        ...
        assignedToId: true,
        teamId: true,
        affectedUsers: { where: { userId: user.id }, select: { userId: true } },
      },
    });
    ...
    if (forWrite) {
      ...
      const mayAttach =
        ...
        this.access.canWorkIssue(user, issue, teamMember);
      if (!mayAttach)
        throw new ForbiddenException(
          "You are not authorized to add evidence to this issue.",
        );
    }
    return issue;
  }
  ...
  private async requireMessage(
    ...
    if (
      ...
      throw new ForbiddenException(
        "Only the sender may add an attachment during the message edit window.",
      );
    return message;
  }

  private async requireConversationWrite(
    user: AuthPrincipal,
    ...
    const participant = await this.prisma.conversationParticipant.findFirst({
      where: {
        ...
        userId: user.id,
        leftAt: null,
        conversation: { collegeId: user.collegeId, archivedAt: null },
      },
      select: {
        role: true,
        conversation: { select: { sendRestricted: true } },
      },
    });
    if (!participant) throw new NotFoundException("Conversation not found.");
    if (
      participant.conversation.sendRestricted &&
      participant.role === "MEMBER"
    )
      throw new ForbiddenException(
        "Only group administrators may send attachments in this conversation.",
      );
    return participant;
  }

  private pendingAttachmentView<
    T extends {
      id: string;
      originalName: string;
      mimeType: string;
  ...
  private validateFile(
    ...
    if (
      ...
      throw new BadRequestException("Executable and script files are blocked.");
    const configuredAllowlist = this.config
      .get<string>("MESSAGE_ALLOWED_MIME_TYPES")
      ?.split(",")
      .map((value) => value.trim())
      .filter(Boolean);
    if (
      configuredAllowlist?.length &&
      !configuredAllowlist.includes(input.mimeType)
    )
      throw new BadRequestException(
        "This file type is disabled by the college attachment policy.",
      );
    if (!MIME_EXTENSIONS[input.mimeType]?.includes(extension))
      throw new BadRequestException(
        "File type and extension are not allowed or do not match.",
      );
    const limitMb = input.mimeType.startsWith("image/")
      ? this.config.get<number>("MAX_IMAGE_SIZE_MB", 10)
      : input.mimeType.startsWith("video/")
        ? this.config.get<number>("MAX_VIDEO_SIZE_MB", 50)
        : input.mimeType.startsWith("audio/")
          ? this.config.get<number>("MAX_AUDIO_SIZE_MB", 15)
          : this.config.get<number>("MAX_DOCUMENT_SIZE_MB", 15);
  ...
  private async verifyObject(
    ...
    >,
  ): Promise<{ sha256: string; content: Buffer }> {
    ...
    const content = await this.toBuffer(
      object.Body as Readable | undefined,
      input.sizeBytes,
    );
    if (!this.matchesSignature(input.mimeType, content))
      throw new BadRequestException(
  ...
  private matchesSignature(mimeType: string, content: Buffer): boolean {
    ...
    if (mimeType === "image/gif")
      return ["GIF87a", "GIF89a"].includes(
        content.subarray(0, 6).toString("ascii"),
      );
    ...
    if (["audio/webm", "video/webm"].includes(mimeType))
      return content
        .subarray(0, 4)
  ...
  private async processImage(
    ...
    if (
      ...
      (original.length > 1_000_000 ||
        (metadata.width ?? 0) > 2560 ||
        (metadata.height ?? 0) > 2560)
    ) {
      ...
      if (mimeType === "image/jpeg")
        content = await resized.jpeg({ quality: 84, mozjpeg: true }).toBuffer();
      else if (mimeType === "image/png")
        content = await resized.png({ compressionLevel: 9 }).toBuffer();
      else if (mimeType === "image/webp")
        content = await resized.webp({ quality: 84 }).toBuffer();
      await this.client.send(
        new PutObjectCommand({
          Bucket: this.bucket,
          Key: storageKey,
  ...
  private async scan(
    ...
    if (!response.ok)
      throw new BadRequestException(
        "The malware scanner is unavailable; the attachment was not accepted.",
      );
  ...
  private async toBuffer(
    ...
    if (!stream)
      throw new BadRequestException("Uploaded object content is unavailable.");
    const chunks: Buffer[] = [];
    let size = 0;
    ...
    if (size !== expectedSize)
      throw new BadRequestException(
        "Uploaded object size changed during verification.",
      );
