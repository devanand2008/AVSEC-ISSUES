...
export class AttendanceController {
  ...
  @Permissions("attendance.correction.request") @Post("corrections") requestCorrection(@CurrentUser() user: AuthPrincipal, @Body() input: RequestCorrectionDto) { return this.attendance.requestCorrectionForRecord(user, input); }
  ...
  @Get("corrections") corrections(@CurrentUser() user: AuthPrincipal, @Query("status") status?: "PENDING" | "APPROVED" | "REJECTED") { return this.attendance.corrections(user, status); }
  @Get("corrections/:id") correctionDetail(@CurrentUser() user: AuthPrincipal, @Param("id", ParseUUIDPipe) id: string) { return this.attendance.correction(user, id); }
