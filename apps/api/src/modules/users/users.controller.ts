...
import { ArchiveUserDto, AssignUserRoleDto, BulkPeopleDto, CreateMaintenanceStaffDto, CreateRoleDto, CreateUserDto, DeleteUserDto, NotificationPreferencesDto, RemoveUserRoleDto, ResetUserPasswordDto, UpdateRoleDto, UpdateUserAccessDto, UpdateUserStatusDto } from "./dto/user.dto";
...
export class UsersController {
  ...
  @Get(["profile/me", "users/me/profile", "students/me/profile"]) myProfile(@CurrentUser() user: AuthPrincipal) { return this.users.myProfile(user); }
  @Get("profile/me/status") myProfileStatus(@CurrentUser() user: AuthPrincipal) { return this.users.myProfileStatus(user); }
  @Patch("profile/me/notification-preferences") notificationPreferences(@CurrentUser() user: AuthPrincipal, @Body() input: NotificationPreferencesDto, @Req() request: RequestWithId) { return this.users.updateNotificationPreferences(user, input, request.id); }
  @Patch(["profile/me", "users/me/profile", "students/me/profile"]) saveMyProfile(@CurrentUser() user: AuthPrincipal, @Body() input: Record<string, unknown>, @Req() request: RequestWithId) { return this.users.saveMyProfileDraft(user, input, request.id); }
  @Post(["profile/me/submit", "users/me/profile/submit", "students/me/profile/submit"]) submitMyProfile(@CurrentUser() user: AuthPrincipal, @Body() input: Record<string, unknown>, @Req() request: RequestWithId) { return this.users.submitMyProfile(user, input, request.id); }
  ...
  @Permissions("users.read") @Get(["admin/users", "admin/students", "admin/profiles", "admin/profile-submissions", "admin/people"]) adminList(
  ...
  @Permissions("users.read") @Get("admin/people/filter-options") peopleFilterOptions(
    @CurrentUser() user: AuthPrincipal,
    @Query("departmentId") departmentId?: string,
    @Query("roomSearch") roomSearch?: string,
  ) { return this.users.peopleFilterOptions(user, departmentId, roomSearch); }
  ...
  @Permissions("users.update") @Patch(["admin/users/:publicId", "admin/students/:publicId", "admin/profiles/:publicId", "admin/people/:publicId"]) adminUpdate(@CurrentUser() user: AuthPrincipal, @Param("publicId", ParseUUIDPipe) publicId: string, @Body() input: Record<string, unknown>, @Req() request: RequestWithId) { return this.users.updateBasic(user, publicId, input, request.id); }
  ...
  @Permissions("users.suspend") @Post(["admin/users/:publicId/suspend", "admin/students/:publicId/suspend", "admin/people/:publicId/suspend"]) adminSuspend(@CurrentUser() user: AuthPrincipal, @Param("publicId", ParseUUIDPipe) publicId: string, @Body() input: Partial<UpdateUserStatusDto>, @Req() request: RequestWithId) { return this.users.status(user, publicId, { status: AccountStatus.SUSPENDED, reason: input.reason || "Suspended by admin" }, request.id); }
  ...
  @Permissions("users.delete_permanent") @Delete(["admin/users/:publicId", "admin/people/:publicId", "admin/people/:publicId/permanent"]) deletePerson(@CurrentUser() user: AuthPrincipal, @Param("publicId", ParseUUIDPipe) publicId: string, @Body() input: DeleteUserDto, @Req() request: RequestWithId) { return this.users.deletePermanently(user, publicId, input, request.id); }
  @Permissions("users.suspend") @Post("admin/people/bulk/archive") bulkArchive(@CurrentUser() user: AuthPrincipal, @Body() input: BulkPeopleDto, @Req() request: RequestWithId) { return this.users.bulkStatus(user, input.ids, AccountStatus.ARCHIVED, input.reason, request.id); }
  @Permissions("users.suspend") @Post("admin/people/bulk/restore") bulkRestore(@CurrentUser() user: AuthPrincipal, @Body() input: BulkPeopleDto, @Req() request: RequestWithId) { return this.users.bulkStatus(user, input.ids, AccountStatus.ACTIVE, input.reason, request.id); }
  @Permissions("users.suspend") @Post("admin/people/bulk/suspend") bulkSuspend(@CurrentUser() user: AuthPrincipal, @Body() input: BulkPeopleDto, @Req() request: RequestWithId) { return this.users.bulkStatus(user, input.ids, AccountStatus.SUSPENDED, input.reason, request.id); }
  ...
  @Permissions("users.create") @Post(["admin/profiles/:publicId/verify", "admin/users/:publicId/verify-profile", "admin/students/:publicId/verify-profile"]) adminVerifyProfile(@CurrentUser() user: AuthPrincipal, @Param("publicId", ParseUUIDPipe) publicId: string, @Req() request: RequestWithId) { return this.users.verifyProfile(user, publicId, request.id); }
  @Permissions("users.create") @Post(["admin/profiles/:publicId/reject", "admin/users/:publicId/reject-profile", "admin/students/:publicId/reject-profile"]) adminRejectProfile(@CurrentUser() user: AuthPrincipal, @Param("publicId", ParseUUIDPipe) publicId: string, @Body("reason") reason: string, @Req() request: RequestWithId) { return this.users.rejectProfile(user, publicId, reason, request.id); }
