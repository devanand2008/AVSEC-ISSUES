...
import type {
  DataMaintenanceCategory,
  DataMaintenanceDryRunDto,
  ExecuteDataMaintenanceDto,
} from "./dto/admin.dto";
...
export class DataMaintenanceService {
  ...
  categories() {
    return [
      {
        code: "ARCHIVE_OLD_ACADEMIC_YEAR",
        destructive: false,
        requires: ["academicYearId"],
      },
      {
        code: "PROMOTE_STUDENTS",
        destructive: false,
        requires: ["sourceSectionId", "targetSectionId"],
      },
      {
        code: "MOVE_STUDENTS_TO_NEW_SEMESTER",
        destructive: false,
        requires: ["sourceSectionId", "targetSectionId"],
      },
      {
        code: "ARCHIVE_OLD_ATTENDANCE_SESSIONS",
        destructive: false,
        requires: ["beforeDate"],
      },
      {
        code: "ARCHIVE_OLD_COURSE_ASSIGNMENTS",
        destructive: false,
        requires: ["beforeDate"],
      },
      {
        code: "ARCHIVE_OLD_ANNOUNCEMENTS",
        destructive: false,
        requires: ["beforeDate"],
      },
      {
        code: "ARCHIVE_CLOSED_ISSUES",
        destructive: false,
        requires: ["beforeDate"],
      },
      {
        code: "DELETE_TEMPORARY_IMPORTS",
        destructive: true,
        requires: ["beforeDate"],
      },
      {
        code: "CLEAN_EXPIRED_SESSIONS",
        destructive: true,
        requires: ["beforeDate"],
      },
      {
        code: "CLEAN_ORPHANED_ATTACHMENTS",
        destructive: true,
        requires: ["beforeDate"],
      },
      {
        code: "PREPARE_NEW_ACADEMIC_YEAR",
        destructive: false,
        requires: ["targetAcademicYearId"],
      },
    ];
  }
  ...
  async dryRun(
    user: AuthPrincipal,
    input: DataMaintenanceDryRunDto,
    requestId: string,
  ) {
  ...
  async registerBackup(
    ...
    if (job.executedAt)
      throw new ConflictException(
        "This maintenance job has already been executed.",
      );
    if (job.status !== "ANALYSED" && job.status !== "BACKUP_CONFIRMED")
      throw new ConflictException(
        "The maintenance job is not ready for backup confirmation.",
      );
    const reference = backupReference.trim();
    await this.requireVerifiedBackup(user.collegeId, reference, job.createdAt);
    const updated = await this.prisma.$transaction(async (tx) => {
      const saved = await tx.dataMaintenanceJob.update({
        where: { id: job.id },
        data: { backupReference: reference, status: "BACKUP_CONFIRMED" },
      });
      await this.audit.record(
        {
          actorId: user.id,
    ...
    return {
      ...
      backupReference: updated.backupReference,
    };
  }
  ...
  async execute(
    user: AuthPrincipal,
    jobId: string,
    input: ExecuteDataMaintenanceDto,
    requestId: string,
  ) {
    this.requireMaintainer(user);
    const job = await this.requireJob(user, jobId);
    if (job.executedAt || job.status === "COMPLETED") return this.view(job);
    if (job.status !== "BACKUP_CONFIRMED" || !job.backupReference)
      throw new BadRequestException(
        "Register a verified backup reference before execution.",
      );
    if (job.backupReference !== input.backupReference.trim())
      throw new BadRequestException(
        "Backup reference does not match the dry-run job.",
      );
    await this.requireVerifiedBackup(
      user.collegeId,
      job.backupReference,
      job.createdAt,
    );
    if (
      !job.confirmationPhraseHash ||
      !this.matchesHash(
    ...
    const currentCounts = await this.analyse(
      user,
      job.category as DataMaintenanceCategory,
      parameters,
    );
    if (
      JSON.stringify(currentCounts) !==
      JSON.stringify(this.object(job.recordCounts))
    ) {
      throw new ConflictException(
        "The matching record counts changed after the dry run. Run a new analysis before executing.",
      );
    }

    const result = await this.executeCategory(
      user,
      job.category as DataMaintenanceCategory,
      parameters,
    );
  ...
  private async analyse(
    user: AuthPrincipal,
    category: DataMaintenanceCategory,
    ...
    switch (category) {
      case "ARCHIVE_OLD_ACADEMIC_YEAR": {
        ...
        return {
          ...
          attendanceRecords: await this.prisma.attendanceRecord.count({
            where: { session: { academicYearId: year.id } },
          }),
          attendanceSummaries: await this.prisma.attendanceSummary.count({
            where: { section: { semester: semesterWhere } },
          }),
        };
      }
      ...
      case "ARCHIVE_OLD_COURSE_ASSIGNMENTS":
        return {
          facultySubjectAssignments:
            await this.prisma.facultySubjectAssignment.count({
              where: {
                isActive: true,
                validUntil: { lt: before },
                section: {
                  semester: { programme: { collegeId: user.collegeId } },
                },
              },
            }),
          classStaffAssignments: await this.prisma.classStaffAssignment.count({
            where: {
              isActive: true,
              validUntil: { lt: before },
              section: {
          ...
          coordinatorAssignments:
            await this.prisma.classCoordinatorAssignment.count({
              where: {
                isActive: true,
                validUntil: { lt: before },
                section: {
      ...
      case "ARCHIVE_OLD_ANNOUNCEMENTS":
        return {
          announcements: await this.prisma.announcement.count({
            where: {
              collegeId: user.collegeId,
              archivedAt: null,
              OR: [
      ...
      case "ARCHIVE_CLOSED_ISSUES":
        return {
          issues: await this.prisma.issue.count({
            where: {
              collegeId: user.collegeId,
              archivedAt: null,
      ...
      case "DELETE_TEMPORARY_IMPORTS": {
        ...
        return {
          ...
          importRows: importIds.length
            ? await this.prisma.importJobRecord.count({
                where: { importJobId: { in: importIds } },
              })
            : 0,
          failedAttendanceImports:
            await this.prisma.attendanceImportBatch.count({
              where: {
                collegeId: user.collegeId,
      ...
      case "CLEAN_EXPIRED_SESSIONS":
        return {
          ...
          refreshTokens: await this.prisma.refreshToken.count({
            where: {
              session: {
                user: { collegeId: user.collegeId },
                expiresAt: { lt: before },
      ...
      case "CLEAN_ORPHANED_ATTACHMENTS":
        return {
          pendingAttachmentUploads:
            await this.prisma.messageAttachmentUpload.count({
              where: {
                collegeId: user.collegeId,
                status: { not: "CONSUMED" },
                expiresAt: { lt: before },
      ...
      case "PREPARE_NEW_ACADEMIC_YEAR": {
        const target = await this.academicYear(
          user,
          parameters.targetAcademicYearId,
        );
        return {
          targetAcademicYears: 1,
          currentAcademicYearsToClose: await this.prisma.academicYear.count({
            where: {
              collegeId: user.collegeId,
              isCurrent: true,
              id: { not: target.id },
          ...
          targetSemesters: await this.prisma.semester.count({
            where: { academicYearId: target.id },
          }),
          targetSections: await this.prisma.section.count({
            where: { semester: { academicYearId: target.id } },
          }),
        };
      }
    }
  ...
  private async executeCategory(
    ...
    switch (category) {
        ...
        return this.prisma.$transaction(
          async (tx) => {
            ...
            await tx.subject.updateMany({
              where: { semester: semesterWhere },
              data: { isActive: false },
            });
            const sections = await tx.section.updateMany({
              where: { semester: semesterWhere, archivedAt: null },
            ...
            await tx.archivedRecord.create({
              data: {
                ...
                entityId: year.id,
                reason:
                  "Archived by the controlled academic-year maintenance workflow.",
                archivedById: user.id,
                metadata: {
                  attendanceSessions: attendance.count,
      ...
      case "ARCHIVE_OLD_ATTENDANCE_SESSIONS": {
        ...
        return {
          attendanceSessions: updated.count,
          recordsDeleted: 0,
          historicalAttendancePreserved: true,
        };
      }
      case "ARCHIVE_OLD_COURSE_ASSIGNMENTS": {
        const [faculty, staff, coordinators] = await this.prisma.$transaction([
          this.prisma.facultySubjectAssignment.updateMany({
            where: {
              isActive: true,
              validUntil: { lt: before },
              section: {
          ...
          this.prisma.classStaffAssignment.updateMany({
            where: {
              isActive: true,
              validUntil: { lt: before },
              section: {
          ...
          this.prisma.classCoordinatorAssignment.updateMany({
            where: {
              isActive: true,
              validUntil: { lt: before },
              section: {
        ...
        return {
          facultySubjectAssignments: faculty.count,
          classStaffAssignments: staff.count,
          coordinatorAssignments: coordinators.count,
        };
      }
      case "ARCHIVE_OLD_ANNOUNCEMENTS": {
      ...
      case "DELETE_TEMPORARY_IMPORTS": {
        ...
        if (storage.failed)
          throw new ConflictException(
            "Some temporary import objects could not be removed from private storage. Retry the cleanup.",
          );
        const result = await this.prisma.$transaction(async (tx) => {
          const rows = jobs.length
            ? await tx.importJobRecord.deleteMany({
                where: { importJobId: { in: jobs.map((job) => job.id) } },
              })
            : { count: 0 };
          const deletedJobs = jobs.length
            ...
            : { count: 0 };
          const attendance = await tx.attendanceImportBatch.deleteMany({
            where: {
              collegeId: user.collegeId,
              status: { in: ["FAILED", "ROLLED_BACK"] },
              createdAt: { lt: before },
          ...
          return {
            ...
            failedAttendanceImports: attendance.count,
          };
        });
      ...
      case "CLEAN_EXPIRED_SESSIONS": {
        ...
        return {
          ...
          refreshTokensRemovedByCascade: true,
        };
      }
      case "CLEAN_ORPHANED_ATTACHMENTS": {
        const uploads = await this.prisma.messageAttachmentUpload.findMany({
          where: {
            collegeId: user.collegeId,
            status: { not: "CONSUMED" },
            expiresAt: { lt: before },
        ...
        const keys = uploads.flatMap((upload) =>
          [upload.storageKey, upload.thumbnailKey].filter(
            (key): key is string => Boolean(key),
          ),
        );
        const storage = await this.storage.deleteMaintenanceObjects(keys);
        if (storage.failed)
          throw new ConflictException(
            "Some expired attachment objects could not be removed from private storage. Retry the cleanup.",
          );
        const deleted = uploads.length
          ? await this.prisma.messageAttachmentUpload.deleteMany({
      ...
      case "PREPARE_NEW_ACADEMIC_YEAR": {
        ...
        return this.prisma.$transaction(
          async (tx) => {
            const previous = await tx.academicYear.updateMany({
              where: {
                collegeId: user.collegeId,
                isCurrent: true,
                id: { not: target.id },
            ...
            await tx.section.updateMany({
              where: {
                semester: { academicYearId: target.id },
                archivedAt: null,
              },
              data: { isActive: true },
            });
            return {
              ...
              previousCurrentYearsClosed: previous.count,
              prepared: true,
            };
          },
          { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
  ...
  private async transferAnalysis(
    user: AuthPrincipal,
    parameters: MaintenanceParameters,
  ): Promise<CountReport> {
    ...
    return {
      students,
      targetCurrentStudents: targetStudents,
      targetCapacity: target.capacity,
      targetAvailableAfterMove: target.capacity - targetStudents - students,
    };
  }
  ...
  private async transferStudents(
    ...
    return this.prisma.$transaction(
      async (tx) => {
        const lockedTarget = await this.placements.lockActiveSection(
          tx,
          user.collegeId,
          target.id,
        );
        ...
        await this.placements.assertAvailable(
          tx,
          lockedTarget,
          profiles.length,
        );
        ...
        await tx.userScope.deleteMany({
          where: { userId: { in: userIds }, scopeType: "SECTION" },
        });
        await tx.userScope.createMany({
          data: userIds.map((userId) => ({
            userId,
            scopeType: "SECTION" as const,
            scopeId: lockedTarget.id,
          })),
        });
        return {
          studentsMoved: userIds.length,
          sourceSectionId: source.id,
          targetSectionId: lockedTarget.id,
          historicalAttendancePreserved: true,
          previousMembershipsArchived: true,
        };
      },
      { isolationLevel: Prisma.TransactionIsolationLevel.Serializable },
    );
  ...
  private async transferSections(
    user: AuthPrincipal,
    parameters: MaintenanceParameters,
  ) {
    if (!parameters.sourceSectionId || !parameters.targetSectionId)
      throw new BadRequestException("Source and target sections are required.");
    if (parameters.sourceSectionId === parameters.targetSectionId)
      throw new BadRequestException(
    ...
    const select = {
      ...
      semester: {
        select: {
          academicYearId: true,
          programmeId: true,
          programme: { select: { collegeId: true, departmentId: true } },
        },
      },
    } as const;
    ...
    if (!source || !target)
      throw new NotFoundException(
        "Source or active target section was not found.",
      );
    if (
      parameters.targetAcademicYearId &&
      parameters.targetAcademicYearId !== target.semester.academicYearId
    )
      throw new BadRequestException(
        "Target academic year must match the target section.",
      );
  ...
  private async deletableImportIds(collegeId: string, before: Date) {
    ...
    const referenced = await this.prisma.user.findMany({
      where: { importBatchId: { in: jobs.map((job) => job.id) } },
      select: { importBatchId: true },
    });
    const retained = new Set(
      referenced.map((row) => row.importBatchId).filter(Boolean),
    );
  ...
  private beforeDate(
    ...
    if (!parameters.beforeDate)
      throw new BadRequestException("A cutoff date is required.");
    const date = new Date(parameters.beforeDate);
    if (Number.isNaN(date.getTime()) || date >= new Date())
      throw new BadRequestException(
        "Cutoff date must be a valid date in the past.",
      );
    return date;
  }
  ...
  private requireMaintainer(user: AuthPrincipal) {
    if (
      !user.permissions.includes("data.maintenance") ||
      !user.roles.some((role) => ["MAIN_ADMIN", "SUPER_ADMIN"].includes(role))
    ) {
      throw new ForbiddenException(
        "Only Main Admin can run controlled data-maintenance workflows.",
      );
    }
  }

  private async requireJob(user: AuthPrincipal, id: string) {
    const job = await this.prisma.dataMaintenanceJob.findFirst({
      where: { id, collegeId: user.collegeId },
    });
  ...
  private async requireVerifiedBackup(
    collegeId: string,
    backupReference: string,
    createdAt: Date,
  ) {
    if (!backupReference)
      throw new BadRequestException(
        "A restore-tested backup reference is required.",
      );
    ...
    if (!backup || backup.restoreTests[0]?.status !== "PASSED")
      throw new BadRequestException(
        "Select a restore-tested backup created after this maintenance analysis.",
      );
    return backup;
  }
