...
function academicLookupMocks(capacity = 70) {
  return {
    ...
    programme: {
      findMany: jest
        .fn()
        .mockResolvedValue([{ id: "programme-1", code: "BTECH-CSE" }]),
    },
    academicYear: {
      findMany: jest
        .fn()
        .mockResolvedValue([{ id: "year-1", name: "2026-27" }]),
    },
    semester: {
      findUnique: jest
        .fn()
        .mockResolvedValue({ id: "semester-1", isActive: true }),
    },
    section: {
...
describe("ImportsHandlerService student academic import", () => {
  it("infers a single active programme and persists register number plus membership academic year", async () => {
    ...
    expect(
      academic.programme.findMany.mock.calls[0]?.[0]?.where.code,
    ).toBeUndefined();
  });
  ...
  it("accepts every Engineering study year from 1 through 4", () => {
    ...
    expect(
      Array.from({ length: 4 }, (_, index) =>
        importStudyYear(String(index + 1)),
      ),
    ).toEqual([1, 2, 3, 4]);
    for (const invalid of ["5", "8", "9"]) {
      expect(() => importStudyYear(invalid)).toThrow(
  ...
  it("removes placement membership and scope before rolling back an imported student", async () => {
    ...
    expect(
      tx.sectionMembership.deleteMany.mock.invocationCallOrder[0],
